# schedule-job
项目采用springboot框架，集成了分布式任务调度框架Quartz。使用SpringMVC作为路由控制，JSON 数据解析使用fastjson 替换了 jackson。HTTP接口调用使用OKhttp3 替代了 HttpClient。 Thrift调用实现了client池化管理。数据持久层集成Mybatis框架，并基于Spring AbstractRoutingDataSource类 结合自定义注解@TargetDataSource实现了数据库读写分离。
